...
public class TruyVanNangCao {
	...
	public void Query9(String event) {
		String q = "Match (per:Person)-[:THAM_GIA]->(eve:Event) WHERE eve.Nhan = {event}"
				+" RETURN per.Nhan AS perNhan";
		System.out.println(" ");
   	 ...
   	 try (Session session = Connection.connection.driver.session()){
            ...
            {
                Record record = result.next();
                System.out.println(record.get("perNhan"));
                i++;
            } 
        }
	}
